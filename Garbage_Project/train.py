...
class GomiConfig(Config):
    ...
    NUM_CLASSES = 1 + 7  # Background + gomi
...
class GomiDataset(utils.Dataset):
    ...
    def load_gomi(self, dataset_dir, subset):
...
def train(model):
    ...
    dataset_train.load_gomi(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = GomiDataset()
    dataset_val.load_gomi(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect gomi class.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/gomi/dataset/",
                        help='Directory of the gomi dataset')
